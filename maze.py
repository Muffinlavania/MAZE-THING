...
def spikeem(which,multi=False,chart=[],chart2=[],dis=False):
  ...
  down=downs2.copy()
  try:
    while (nextt in goods or nextt in linez and tr==1 or nextt in spikez) or (z in listoweird and which in listoweird[z]) or nextt in ['q','x'] and nextt !='s':
      countp+=1
      for i in y:
        if da==True:
          da=False
        ...
        if tr==1:
          if replace!='q' and nextt!='q' and mazeq[i+defi]!='q':
            if not (replace=='-' and (z in listoweird and which in listoweird[z])): 
      ...
      for i in range(5):
        if y_t=='up':
          ...
          if not (z in listoweird and which in listoweird[z]):
            time.sleep(.05)
          else:
...
while True:
  ...
  if keyz=='e': #Yes testing
    if 'sam' in name or 'srn' in name or name in ['Muffinlavania','squidcoder']:
      nextlevel()
  if oooG==True:
    oooG=False
